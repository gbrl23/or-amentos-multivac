...
export default function DashboardLayout() {
  ...
  const menuItems = [
    ...
    { path: '/orcamentos', icon: FileText, label: 'Orçamentos (App)' },
    { path: '/dashboard/catalogo', icon: Box, label: 'Consultar Caixa/Itens' }, // Example of custom rename
    { path: '/dashboard/catalogo', icon: Package, label: 'Gerenciar Catálogo' },
    { path: '/dashboard/catalogo/upload', icon: Upload, label: 'Importar Catálogo' },
    { path: '/usuarios', icon: Users, label: 'Equipe / Usuários' },
  ];
    ...
    <div className="min-h-screen bg-[#f8fafc] flex font-sans text-gray-900">
      ...
      <aside className="w-64 bg-white border-r border-gray-200 flex flex-col fixed inset-y-0 left-0 z-20 shadow-sm">
        ...
        <nav className="flex-1 overflow-y-auto py-6 px-4 space-y-1">
          <div className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-4 px-2">Navegação Principal</div>
      ...
      <main className="flex-1 flex flex-col ml-64 min-h-screen relative overflow-x-hidden">
        ...
        <header className="h-16 bg-white/80 backdrop-blur-md border-b border-gray-200 flex items-center justify-between px-8 sticky top-0 z-10">
          <div className="flex items-center text-sm text-gray-500 font-medium">
             {/* Pao de migalhas dinamico */}
             {location.pathname === '/dashboard' && 'Visão Geral do Painel'}
             {location.pathname === '/dashboard/catalogo' && 'Catálogo de Produtos'}
             {location.pathname === '/dashboard/catalogo/upload' && 'Importação de Dados'}
             {location.pathname === '/usuarios' && 'Gerenciamento de Equipe'}
             {location.pathname === '/orcamentos' && 'Sistema de Orçamentos'}
             {location.pathname === '/perfil' && 'Perfil e Preferências'}
          </div>
